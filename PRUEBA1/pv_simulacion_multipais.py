...
paises = {
    "Chile": os.path.join(os.path.dirname(os.path.abspath(__file__)), "datos_Chile.csv"),
    "China": os.path.join(os.path.dirname(os.path.abspath(__file__)), "datos_China.csv"),
    "Sudafrica": os.path.join(os.path.dirname(os.path.abspath(__file__)), "datos_Sudafrica.csv")
}

# Capacidades del sistema a simular (en kW)
capacidades = [1000, 5000, 10000, 50000, 100000]  # 1MW, 5MW, 10MW, 50MW, 100MW

# Parámetros técnicos y económicos base
dc_ac_ratio = 1.2                 # Ratio DC/AC
tilt = 25                         # Inclinación de los paneles en grados (25° para Chile)
azimuth = 0                       # Azimut en grados (0° = norte, para hemisferio sur)
...
def simular_sistema_pv(pais, archivo_recurso, capacidad_kw):
    ...
    pv_model.AdjustmentFactors.constant = 1  # Agregar el ajuste constante requerido
...
def analisis_sensibilidad_tilt(pv_model, directorio, pais, capacidad_kw):
    ...
    inclinaciones = [0, 5, 10, 15, 20, 22, 24, 26, 28, 30, 35, 40]
    energias = []
    ...
    for tilt_value in inclinaciones:
        ...
        test_model.AdjustmentFactors.constant = 1  # Agregar el ajuste constante requerido
